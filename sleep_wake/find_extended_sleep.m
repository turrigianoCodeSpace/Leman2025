...
MIN_NUM_STATES = 1;
disp(['Rejecting episodes with fewer than: ',num2str(MIN_NUM_STATES),' states'])
disp(['Max interruption in seconds: ',num2str(short_thresh)])

% convert to seconds
dur_thresh_sec = dur_thresh * 60;

% get state codes and durations
codes = st(:,1); %1 = REM, 2 = NREM, 4 = AW, 5 = QW
durations = diff(st(:,2));
...
for cc = 2:size(new_codes,1)-1
    % if state is WAKE
    if new_codes(cc) == 400 && new_codes(cc-1) < 300
%%         Original code only checked interruptions of one state
...
%             end
%         end
        int_dur = durations(cc);
        wake_still = 1;
        cc_temp = cc;
        while (wake_still==1) && (cc_temp < (size(new_codes,1) - 1))
            cc_temp = cc_temp + 1;
            if new_codes(cc_temp) == 400
                wake_still = 1;
                int_dur = int_dur + durations(cc_temp);
            else
                wake_still = 0;
            end
        end

        if int_dur <= short_thresh
            % do not count it as wake (give it a code of 250, in between wake and sleep)
            new_codes(cc) = 250;
        end
    end
...
for xx = 1:size(tmp_idx,1)-1
    % calculate duration
    try
        duration = st(tmp_idx(xx,2),2) - st(tmp_idx(xx,1),2);
    catch
        keyboard;
    end
...
% REM)
ext_sleep_idx = tmp_good(diff_idx>=MIN_NUM_STATES,:);
