...
for cc = 2:size(new_codes,1)-1
    % if state is SLEEP
    if new_codes(cc) == 100 && new_codes(cc-1) > 100
...
%         end
        ...
        sleep_still = 1;
        cc_temp = cc;
        while (sleep_still==1) && (cc_temp < (size(new_codes,1) - 1))
            cc_temp = cc_temp + 1;
            if new_codes(cc_temp) == 100
                sleep_still = 1;
...
% AW for instance)
ext_wake_idx = tmp_good(diff_idx>=MIN_NUM_STATES,:);
