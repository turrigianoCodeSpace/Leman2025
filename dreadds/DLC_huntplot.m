...
if type == 2 && hs <= 4
    ...
    wrs_hs1 = ranksum(ctl_mat(:,1),dr_mat(:,1))
    wrs_hs2 = ranksum(ctl_mat(:,2),dr_mat(:,2))
    wrs_hs3 = ranksum(ctl_mat(:,3),dr_mat(:,3))

    disp('Comparing Session 1 to Session 3')
    wsr_ctl1v3 = signrank(ctl_mat(:,1),ctl_mat(:,3))
    wsr_dr1v3 = signrank(dr_mat(:,1),dr_mat(:,3))
